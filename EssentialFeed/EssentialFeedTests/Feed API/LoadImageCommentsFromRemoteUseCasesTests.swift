//
//  LoadImageCommentsFromRemoteUseCasesTests.swift
//  EssentialFeedTests
//
...
class LoadImageCommentsFromRemoteUseCasesTests: XCTestCase {
    ...
    private func failure(_ error: RemoteImageCommentsLoader.Error) -> FeedImageDataLoader.Result {
        return .failure(error)
    }

    private func expect(_ sut: RemoteImageCommentsLoader, toCompleteWith expectedResult: FeedImageDataLoader.Result, when action: () -> Void, file: StaticString = #file, line: UInt = #line) {
        let url = URL(string: "https://a-given-url.com")!
